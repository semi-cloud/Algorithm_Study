...
#### 1780
 + N×N 크기의 행렬로 표현되는 종이의 각 칸에는 -1, 0, 1의 세 값 중 하나가 저장되어 있음
   + 종이가 모두 같은 수가 아닌 경우에만 9개로 다시 자르는걸 반복하고, 같은 수면 종이 그대로 사용하는 규칙
 
 > :pencil2: 풀이 방법
   + 이차원 배열에서,  (현재 있는 위치 + N )만큼의 종이 크기가 같은 수로 되어있는지 검사
   + `검사 결과가 false` : 다른 수가 존재하기 때문에, 9 등분으로 분할(N/=3) 한 후 분할된 종이의 각 시작위치에서 모두 재귀 돌리기
   + `검사 결과가 true` : 모두 같은 수이기 때문에, 종이에 저장되어 있는 해당 숫자++
